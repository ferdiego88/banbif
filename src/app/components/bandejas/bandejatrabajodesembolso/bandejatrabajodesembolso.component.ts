...
@Component({
  selector: 'app-bandejatrabajodesembolso',
  templateUrl: './bandejatrabajodesembolso.component.html',
  styleUrls: ['./bandejatrabajodesembolso.component.scss']
})
export class BandejatrabajodesembolsoComponent extends FormularioBase implements OnInit {
  ...
  public irPaginaSolicitud(elemento: any) {
    const url = environment.getRutaBaseApp() + "/hipotecario/solicitud/" + elemento.Id + "/b2";
  ...
  async getSolicitudesPaged(): Promise<EBandejaSolicitud[]> {
    ...
    if (this.paginator.pageIndex == 0 || this.tableQuery.order != order || this.tableQuery.direction != desc || this.tableQuery.pagesize != this.paginator.pageSize) {
      ...
      this.solicitudes_paged = await this.solicitudesService.getBandejaTrabajoDesembolso(filter, order, direction, this.paginator.pageSize, this.datosMaestrosBandeja.currentUser).then();
  ...
  exportarExcel() {
    ...
    this.solicitudesService.getBandejaTrabajoDesembolso(filter, order, direction, 100000, this.datosMaestrosBandeja.currentUser).then(
      (data: PagedItemCollection<any[]>) => {
        ...
        this.excelService.excelListadoBandejaTrabajoGestor('Bandeja de Trabajo Desembolso', 'BandejaTrabajoDesembolso', headers, details);
        this.ocultarProgreso();
        this.isCargando = false;
      },
      err => this.guardarLog(err)
