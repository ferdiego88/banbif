...
@Component({
  selector: 'app-bandejatrabajogestorlegal',
  templateUrl: './bandejatrabajogestorlegal.component.html',
  styleUrls: ['./bandejatrabajogestorlegal.component.scss']
})
export class BandejatrabajogestorlegalComponent extends FormularioBase implements OnInit {
  ...
  public irPaginaSolicitud(elemento: any) {
    const url = environment.getRutaBaseApp() + "/hipotecario/solicitud/" + elemento.Id + "/b5";
    window.open(url, '_blank');
  }
  ...
  async getSolicitudesPaged(): Promise<EBandejaSolicitud[]> {
    ...
    if (this.paginator.pageIndex == 0 || this.tableQuery.order != order || this.tableQuery.direction != desc || this.tableQuery.pagesize != this.paginator.pageSize) {
      ...
      this.solicitudes_paged = await this.solicitudesService.getBandejaTrabajoLegal(filter, order, direction, this.paginator.pageSize,  false, false, true).then();
  ...
  exportarExcel() {
    ...
    this.solicitudesService.getBandejaTrabajoLegal(filter, order, direction, 4999, false, false, true).then(
      (data: PagedItemCollection<any[]>) => {
        ...
        this.excelService.excelListadoBandejaTrabajo('Bandeja de Trabajo Gestor', 'BandejaTrabajoGestor', headers, details);
        this.ocultarProgreso();
        this.isCargando = false;
      },
      err => this.guardarLog(err)
