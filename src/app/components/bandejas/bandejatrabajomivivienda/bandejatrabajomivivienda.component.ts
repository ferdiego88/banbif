...
@Component({
  selector: 'app-bandejatrabajomivivienda',
  templateUrl: './bandejatrabajomivivienda.component.html',
  styleUrls: ['./bandejatrabajomivivienda.component.scss']
})
export class BandejatrabajomiviviendaComponent extends FormularioBase implements OnInit {
  ...
  constructor(
    ...
    super('Bandeja de Trabajo Mi Vivienda', applicationRef, dialog, route, router, masterService, zone, _spinner);
  ...
  ngOnInit(): void {
    ...
    this.obtenerMaestrosYDatos().then(() => {
      ...
      debugger;
      if (!this.datosMaestrosBandeja.PertenceGrupo_U_MiVivienda) {
  ...
  async getSolicitudesPaged(): Promise<EBandejaSolicitud[]> {
    ...
    if (this.paginator.pageIndex == 0 || this.tableQuery.order != order || this.tableQuery.direction != desc || this.tableQuery.pagesize != this.paginator.pageSize) {
      ...
      this.solicitudes_paged = await this.solicitudesService.getBandejaTrabajoLegal(filter, order, direction, this.paginator.pageSize,  false, true, false).then();
  ...
  exportarExcel() {
    ...
    this.solicitudesService.getBandejaTrabajoLegal(filter, order, direction, 4999, false, true, false).then(
      (data: PagedItemCollection<any[]>) => {
        const items: EBandejaSolicitud[] = data.results.map(elemento => {
          return EBandejaSolicitud.parseJson(elemento);
        });

        const headers: string[] = ['N° Solicitud', 'Nro. Documento', 'Nombre Titular', 'Solicitante', 'Fec. Creación', 'Fecha Estado', 'Estado', 'Estado Mi Vivienda', 'Zona', 'Oficina', 'Tipo Producto', 'Moneda', 'Desembolso', 'Oferta', 'Tipo Renta', 'Sustento Ingresos'];
        ...
        this.excelService.excelListadoBandejaTrabajo('Bandeja de Trabajo Mi Vivienda', 'BandejaTrabajoMiVivienda', headers, details);
        this.ocultarProgreso();
        this.isCargando = false;
      },
      err => this.guardarLog(err)
